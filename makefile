...
DISK_ID ?= wwn-0x123456789abcdef0 # replace with your disk ID
...
.PHONY: make-config
make-config: ## Generate install-config.yaml using a helper script.
	@PULL_SECRET='$(PULL_SECRET)' ./scripts/generate-install-config.sh $(CLUSTER_NAME) $(BASE_DOMAIN) $(DISK_ID) $(SSH_KEY)
